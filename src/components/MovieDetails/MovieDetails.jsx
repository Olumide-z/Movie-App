...
const MovieDetails = ({ content }) => {
  ...
  return (
    <Container>
      ...
      <Wrapper>
          <LargeImgWrapper>
            <LargeImg src={`https://image.tmdb.org/t/p/original${content.backdrop_path}`} alt={content.title}/>
            <div className='div'></div>
          </LargeImgWrapper>
          <SmallImgWrapper>
            <SmallImg src={`https://image.tmdb.org/t/p/original${content.poster_path}`} alt={content.title} />
          </SmallImgWrapper>
        <Info>
        ...
        <ReleaseDate>
          <h3>RELEASE DATE </h3>
          <p>{moment(content.release_date || content.first_air_date).format('ll')}</p>
        </ReleaseDate>
        <Rating>
          <h3>RATING</h3> 
          <p>{content.vote_average}<Star /></p>
        </Rating>
        </Info>
      </Wrapper>
      )}
