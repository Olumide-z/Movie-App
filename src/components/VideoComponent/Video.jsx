...
const Video = ({ id, content }) => {
  ...
  const fetchVideos = async () => {
    const { data } = await axios.get(
      `${MOVIE_VIDEO_URL}${id}/videos?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`
    );
  ...
  const fetchVideosTv = async () => {
    const { data } = await axios.get(
      `${TV_VIDEO_URL}${id}/videos?api_key=${process.env.REACT_APP_API_KEY}&language=en-US`
    );
    console.log(data.results[0].key);
    setVideo(data.results[0]);
  };
