...
const MovieCard = (singleMovie) => {
  ...
  return (
    <Container>
      ...
      <MovieDetails>
        <MovieTitle>
          {(singleMovie.title || singleMovie.name) &&
            ((singleMovie.title || singleMovie.name).length <= 20
              ? singleMovie.title || singleMovie.name
              : `${(singleMovie.title || singleMovie.name).slice(0, 20)}...`)}
        </MovieTitle>
        <Rating>
      ...
      <Date>
        Released:{" "}
        {moment(singleMovie.release_date || singleMovie.first_air_date).format(
          "ll"
        )}
      </Date>
      <BtnLink
